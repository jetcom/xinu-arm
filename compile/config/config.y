...
/*
 * Grab all of the scanner prototypes from the header file generated by lex
 * when it generates the config scanner. We need this because there is no
 * global 'lex.h' and different versions of (f)lex have different prototypes.
 */
#include "lex.yy.h"

/* The verbatim section of the config file, if it exists. */
extern char *verbatim_out;
...
int main(int argc, char **argv)
{
    ...
    /*
     * If we have seen the second set of '%%' tokens, copy the rest of the
     * config file into config.h verbatim.
     */
	if (brkcount == 2 )
	{
        if( verbatim_out )
        {
            fputs( verbatim_out, confh );
        }
	}
